...
static int	is_in_quotes(char *str, int index)
{
	...
	while (i < index)
	{
		if ((str[i] == '\'' || str[i] == '"') && (in_quotes == 0 || str[i] == current_quote))
		{
			if (in_quotes)
				in_quotes = 0;
			else
...
static char	*merge_tokens(char **tab, int *i, int size)
{
	...
	merged = strdup(tab[*i]);
	while (*i + 1 < size && ft_strncmp(tab[*i + 1], tab[*i], 1) == 0 && !is_in_quotes(tab[*i], 0))
	{
...
char	**union_tab(char **tab, int size)
{
	...
	while (i < size)
	{
		if (!is_in_quotes(tab[i], 0) && ((ft_strncmp(tab[i], ">", 1) == 0 || ft_strncmp(tab[i], "<", 1) == 0)))
