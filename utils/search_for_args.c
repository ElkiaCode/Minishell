...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/23 22:36:12 by lpolizzi          #+#    #+#             */
/*   Updated: 2025/01/23 22:36:13 by lpolizzi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	handle_argument_types(t_tokens *token, int i, int j)
{
	if (token[i].type[j] == T_ERR && (token[i].type[j + 1] == T_D_QUOTE
		...
		token[i].type[j] = T_ARG;
	else if (token[i].type[j] == T_CMD && token[i].type[j + 1] == T_ERR)
		token[i].type[j + 1] = T_ARG;
	else if (token[i].type[j] == T_ARG && token[i].type[j + 1] == T_ERR)
		token[i].type[j + 1] = T_ARG;
}
